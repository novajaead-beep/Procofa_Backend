...
/// <summary>
/// Fila proyectada de <c>GET /api/users</c> — ya incluye los códigos de rol resueltos, para que
/// Application no tenga que hacer un segundo viaje por usuario. Deliberadamente NO expone
/// <c>password_hash</c>. </summary>
...
/// <summary>Página de resultados de <c>GET /api/users</c> — el total es el conteo SIN paginar, para
/// construir la respuesta <c>{ items, page, pageSize, total }</c>.</summary>
public sealed record UserListPageResult(IReadOnlyList<UserListRow> Items, int Total);

/// <summary>
/// Puerto de acceso a <see cref="User"/>. Deliberadamente NO es un repositorio genérico
/// (<c>IRepository&lt;T&gt;</c> prohibido explícitamente): expone únicamente las operaciones que
/// Auth y la administración de usuarios necesitan. </summary>
public interface IUserRepository
{
    ...
    /// <summary><c>POST /api/users</c>: unicidad de email dentro del tenant (usa la misma
    /// normalización que <see cref="User.Normalize"/>).</summary>
    Task<bool> ExistsByNormalizedEmailAsync(Guid tenantId, string normalizedEmail, CancellationToken cancellationToken);

    /// <summary><c>GET /api/users/{id}</c> y los endpoints de escritura
    /// (status/roles/client-access): carga el usuario CON <see cref="User.Roles"/> y <see
    /// cref="User.ClientAccess"/> ya incluidos (la implementación decide cómo — ej.
    /// <c>AsSplitQuery</c> para evitar el warning de múltiples colecciones). <c>null</c> si no
    /// existe dentro del tenant. </summary>
    Task<User?> GetByIdAsync(Guid tenantId, Guid userId, CancellationToken cancellationToken);

    /// <summary><c>GET /api/users</c>: listado paginado del tenant actual. <paramref
    /// name="search"/> busca en email/first_name/last_name; <paramref name="roleCode"/>si se
    /// provee, ya fue validado contra el catálogo permitido por el caller (Application) — aquí solo
    /// se aplica como filtro. </summary>
